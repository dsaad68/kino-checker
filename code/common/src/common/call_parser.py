...
class CallParser:
    ...
    FILM_ID_PATTERN: re.Pattern[str] = re.compile(r"^([A-Z0-9]+),fid")
    UPCOMING_FILM_PATTERN: re.Pattern[str] = re.compile(r"^([0-9]+),uf")

    DATE_PATTERN: re.Pattern[str] = re.compile(r"(\d{4}-\d{2}-\d{2})")
    TIME_PATTERN: re.Pattern[str] = re.compile(r"(\d{2}:\d{2})")

    DATE_PATTERN_ENDING: re.Pattern[str] = re.compile(r"(\d{4}-\d{2}-\d{2})$")
    ...
    @staticmethod
    def parse_for_input(input_string: str) -> dict[str, Any]:
        """Parses the input string and returns a dictionary of key-value pairs."""
