...
class FilmInfoFinder(DBManager):
    ...
    def check_performance_version_availability(self, film_id: str, versions: dict) -> bool:
        ...
        version_filter = [getattr(Performances, key) == value for key, value in versions.items() if value != 0]
    ...
    def get_performance_dates_by_film_id(self, film_id: str, versions: dict) -> list[date] | None:
        ...
        version_filter = [getattr(Performances, key) == value for key, value in versions.items() if value != 0]
        stmt = select(distinct(Performances.performance_date)).where(
            and_(Performances.film_id == film_id, *version_filter)
        )
        return self.execute_query_all(stmt)

    def get_performance_hours_by_film_id(self, film_id: str, versions: dict, performance_date) -> list[time] | None:
        version_filter = [getattr(Performances, key) == value for key, value in versions.items() if value != 0]
        stmt = select(distinct(Performances.performance_time)).where(
            and_(Performances.film_id == film_id, Performances.performance_date == performance_date, *version_filter)
        )
        return self.execute_query_all(stmt)
    ...
    def get_showing_films_list(self) -> list[str] | None:
    ...
    def _create_upcomings_films_stmt(self) -> select[tuple[str, int]]:
        return select(UpcomingFilms.title, UpcomingFilms.upcoming_film_id).where(
            and_(UpcomingFilms.is_trackable, ~UpcomingFilms.is_released)
        )

    def _create_showing_films_stmt(self) -> select[tuple[str]]:
        return (
