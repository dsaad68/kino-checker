...
def test_update_films_table(db_connection_uri_with_sample_data):
    # Prepare
    exists = {
    ...
    assert (
        wish_film.image_url
        == "https://contentservice.cineorder.shop/contents/img?q=fDJ8M3w3fDEwMjI3OTZfL3JDQ3JHNHN3a3hnRlpmbHVwNTZzeDZ5bWs1aS5qcGdffA"
    )  # type: ignore
    ...
    assert (
        wonka_film.image_url
        == "https://contentservice.cineorder.shop/contents/img?q=fDJ8M3w3fDc4NzY5OV8vZXQxT2ZSdmZ3V21Ua1lpandxYUR3S0ZnWTFsLmpwZ19kZXw"
    )  # type: ignore
    assert start <= wonka_film.last_updated <= end  # type: ignore


def test_update_performances_table(db_connection_uri_with_sample_data):
    # Prepare
    exists = {
...
def test_update_upcoming_table(db_connection_uri_with_sample_data):
    # Prepare
    exists = {
        "title": "Napoleon",
...
def test_update_upcoming_table_cardinality_violation(db_connection_uri_with_sample_data):
    ...
    film_db_manager = FilmDatabaseManager(db_connection_uri_with_sample_data)  # type: ignore
    film_db_manager.update_upcoming_films_table(
        upcoming_films_list=[exists, upsert_case, upsert_case_duplicate, dont_exist]
    )
...
def test_update_released_films_in_upcoming_films_table(db_connection_uri_with_sample_data):
    # Execute
    film_db_manager = FilmDatabaseManager(db_connection_uri_with_sample_data)  # type: ignore
...
def test_update_users_table(db_connection_uri_with_sample_data):
    # Execute
    film_db_manager = FilmDatabaseManager(db_connection_uri_with_sample_data)  # type: ignore
    film_db_manager.update_released_films_in_upcoming_films_table()
...
def test_get_performance_based_on_versions(db_connection_uri_with_sample_data):
    perfomance_1 = PerformanceInfo(
        film_id="58E63000012BHGWDVI",
        performance_id="88D45000023UHQLKCP",
    ...
    perfomance_2 = PerformanceInfo(
        film_id="58E63000012BHGWDVI",
        performance_id="99D45000023UHQLKCP",
...
def test_get_users_to_notify(db_connection_uri_with_sample_data):
    perfomance_1 = PerformanceInfo(
        film_id="58E63000012BHGWDVI",
        performance_id="88D45000023UHQLAAA",
        date=date(2023, 11, 15),
        time=time(20, 0, 0),
        is_imax=True,
        is_ov=True,
    ...
    perfomance_2 = PerformanceInfo(
        film_id="58E63000012BHGWDVI",
        performance_id="99D45000023UHQLAAA",
        date=date(2023, 11, 16),
        time=time(17, 0, 0),
        is_imax=True,
        is_ov=True,
...
def test_update_notified_users_table(db_connection_uri_with_sample_data):
    # Execute
    film_db_manager = FilmDatabaseManager(db_connection_uri_with_sample_data)  # type: ignore
    user_list = film_db_manager.get_users_to_notify()
